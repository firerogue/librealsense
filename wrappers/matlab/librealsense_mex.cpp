...
void make_factory() {
...

    {
        ClassFactory context_factory("rs2::context");
        // This lambda feels like it should be possible to generate automatically with templates
        context_factory.record("new", 1, 0, 0, [](int outc, mxArray* outv[], int inc, const mxArray* inv[])
        {
            outv[0] = MatlabParamParser::wrap(rs2::context());
        });
        context_factory.record("delete", 0, 1, 1, [](int outc, mxArray* outv[], int inc, const mxArray* inv[])
        {
            MatlabParamParser::destroy<rs2::context>(inv[0]);
        });
        context_factory.record("query_devices", 1, (rs2::device_list(rs2::context::*)() const) &rs2::context::query_devices);
        context_factory.record("query_all_sensors", 1, &rs2::context::query_all_sensors);
        context_factory.record("get_sensor_parent", 2, &rs2::context::get_sensor_parent);
        context_factory.record("load_device", 2, &rs2::context::load_device);
        context_factory.record("unload_device", 2, &rs2::context::unload_device);
        context_factory.record("unload_tracking_module", 1, &rs2::context::unload_tracking_module);
        factory->record(context_factory);
    }
